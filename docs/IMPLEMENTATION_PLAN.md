...
| **DEC-20** | `edition_sources` / `edition_source_conflicts` have no schema. | They appear only in the §5.2 entity map and are consumed by the ToC Refresh Agent (§10.4), but no DDL exists anywhere in this document. Needs designing, not just transcribing. Scope: what a "source" is (URL + checksum + fetch time is already on `examination_editions`), and what a conflict between two sources records. | M4 — blocks the §5.6 migration |
